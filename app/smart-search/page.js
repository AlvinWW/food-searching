...
export default function SmartSearch() {
  ...
  const handleSearch = async () => {
    ...
    if (!coords) return alert("請先允許定位，AI 才能幫您找附近的店！")
    ...
    try {
      // 呼叫後端 API (注意：這裡是呼叫 /api/smart-search)
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-purple-50 to-blue-50 p-6 font-sans">
      <div className="max-w-md mx-auto">
        ...
        <div className="bg-white p-2 rounded-[2rem] shadow-xl border border-purple-100 mb-6 transition-all focus-within:ring-2 focus-within:ring-purple-200">
          ...
          <div className="flex justify-between items-center px-2 mt-2">
            <span className="text-xs text-gray-400 px-2">
              {query.length > 0 ? `${query.length} 字` : 'AI 準備就緒'}
            </span>
            <button
          ...
          <div className="space-y-3">
            ...
            {SUGGESTIONS.map((text) => (
              <button
                ...
                className="w-full text-left p-4 bg-white rounded-2xl text-gray-600 text-sm hover:bg-purple-50 hover:text-purple-700 transition-colors shadow-sm border border-transparent hover:border-purple-100 active:scale-[0.98]"
              >
                "{text}"
              </button>
            ))}
        ...
        {/* 錯誤訊息 */}
        {error && (
          <div className="bg-red-50 text-red-500 p-4 rounded-2xl text-center text-sm font-bold border border-red-100 animate-pulse">
            😢 {error}
          </div>
        )}
          ...
          <div className="bg-white rounded-[2rem] shadow-2xl overflow-hidden animate-in zoom-in-50 duration-500 border border-gray-100 mb-10">
            <div className="bg-purple-600 p-6 text-white text-center relative overflow-hidden">
              ...
              <div className="flex justify-center items-center gap-2 mt-2 opacity-90 relative z-10 text-sm">
                <div className="flex items-center bg-white/20 px-2 py-0.5 rounded-lg">
                  <Star fill="white" size={14} className="mr-1"/> {result.rating}
                </div>
                <span>•</span>
            ...
            <div className="p-6">
              ...
              <a 
                ...
                target="_blank"
                className="block w-full bg-gray-900 text-white text-center font-bold py-4 rounded-xl hover:bg-black transition-all shadow-lg shadow-gray-200 active:scale-[0.98]"
              >
                前往 Google Maps
              </a>
            </div>
